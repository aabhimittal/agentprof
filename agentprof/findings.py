...
def findings(totals, steps, blocks, sys_cost, sub_cost):
    ...
    dups = sorted([b for b in blocks if b["dup_max"] > 1 and b["bytes"] >= 512],
                  key=lambda b: -b["repeat_cost"])
    if dups:
        d = dups[0]
        total_dup = sum(b["repeat_cost"] for b in dups)
        detail = (
        ...
        out.append(_f(
            ...
            detail, total_dup, total_dup / cost))
    ...
    heavy = [b for b in blocks
             if b["repeat_cost"] > 0.05 * cost and b["bytes"] >= 512 and b["h"] not in counted]
    for b in sorted(heavy, key=lambda b: -b["repeat_cost"])[:2]:
        out.append(_f(
            "resend-" + b["h"], HIGH,
            "%s was billed %d times" % (b["label"], b["n"]),
            "%.1f KB re-sent on every later call, costing $%.4f (%.0f%% of the run) beyond "
            ...
            % (b["bytes"] / 1024.0, b["repeat_cost"], 100 * b["repeat_cost"] / cost),
            b["repeat_cost"], b["repeat_cost"] / cost))

    # 3. Fixed overhead that is paid on literally every call.
    if sys_cost > 0.25 * cost:
        out.append(_f(
            "system-overhead", MED,
